...
#   Solve the problem argmin_x ||y-Hx||, where if Nc is even the elements of
#   x are integers from [0:Nc-1]*2-(Nc-1).  The input vector y is of
...
# """ ->
function hard_sphere(Y::Array{Td,2},H::Array{Td,2},Nc::Integer) where {Td}
    (N,M) = size(H);
